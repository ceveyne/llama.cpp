...
ggml_cgraph * clip_graph_qwen3vl::build() {
    ...
    auto apply_vision_rope = [&](ggml_tensor * cur, const char * name, int il) -> ggml_tensor * {
        ...
        ggml_tensor * first_rot  = ggml_sub(ctx0, ggml_mul(ctx0, first, cos), ggml_mul(ctx0, second, sin));
        ggml_tensor * second_rot = ggml_add(ctx0, ggml_mul(ctx0, first, sin), ggml_mul(ctx0, second, cos));
